...
func (m *postgresDBRepo) InsertHost(h models.Host) (int, error) {
	...
	//未支持上下文取消操作（context cancelation）的驅動會在查詢完成之後才返回。
...
from hosts where id = $1`
	...
	query = `select 
       			hs.id, hs.host_id, hs.service_id, hs.active, hs.schedule_number, hs.schedule_unit,hs.last_check,
...
func (m *postgresDBRepo) UpdateHost(h models.Host) error {
	...
	stmt := `update hosts set host_name=$1,canonical_name=$2, url=$3, ip=$4, ipv6=$5, os=$6, active=$7, location=$8, updated_at=$9 where id=$10`
...
func (m *postgresDBRepo) GetServicesByStatus(status string) ([]models.HostService, error) {
	...
	query := `
		...
		hs.last_check, hs.status, hs.created_at, hs.updated_at, h.host_name, s.service_name
		from host_services hs
			left join hosts h on (hs.host_id = h.id)
			left join services s on (hs.service_id = s.id)
		where hs.active=1 and status=$1
...
func (m *postgresDBRepo) GetHostServiceByID(id int) (models.HostService, error) {
	...
	if err != nil {
		log.Println("GetHostServiceByID error:", err)
		return hs, err
	}
...
func (m *postgresDBRepo) UpdateHostServiceByHostIdServiceID(hostID, serviceID int) (models.HostService, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
